...
class canvasDust {
  constructor(canvasID) {
    ...
    this.resize = () => {
      ...
      this.ctx.shadowColor = this.ctx.fillStyle = this.color;
    };
    this.stop = () => {
      this.inStop = true;
    };
  ...
  constructor() {
    ...
    this.addEvent = (header) => {
      header.addEventListener("click", (click) => {
        if (click.target.tagName !== "BUTTON" && click.target.tagName !== "A") {
...
class Code {
  constructor() {
    ...
    this.doAsMermaid = (item) => {
      ...
      item.outerHTML =
        '<div class="highlight mermaid">' + Amermaid.innerText + "</div>";
    };
    this.resetName = (str) => {
      if (str == "plaintext") {
        return "TEXT";
    ...
    this.doAsCode = (item) => {
      const codeType = this.resetName(item.classList[1]),
        lineCount =
          getElement(".gutter", item).children[0].childElementCount >> 1;
      item.classList.add(lineCount < 16 ? "open" : "fold");
      item.classList.add("expand-box");
      item.innerHTML = `<div class="ex-header" tabindex='0'>
        <i class="i-status"></i>
        <span class="ex-title">${format(
          config.code.codeInfo,
          codeType,
          lineCount
        )}</span>
      </div>
      ...
      if (codeBlocks !== null) {
        codeBlocks.forEach((item) => {
          if (item.getAttribute("code-find") === null) {
            try {
              if (
                !item.classList.contains("mermaid") &&
                item.querySelector(".code-header") === null
              ) {
...
class Cursor {
  constructor() {
    ...
    this.set = (X = this.nowX, Y = this.nowY) => {
      this.outer.transform = `translate(calc(${X.toFixed(2)}px - 50%),
...
class Index {
  constructor() {
    ...
    this.setHTML = () => {
      try {
        this.headerLink =
          getElement("#post-content").querySelectorAll("h1,h2,h3,h4,h5,h6");
    ...
    getElement("main").addEventListener(
      "scroll",
      () => {
        if (this.tocLink.length) {
          this.modifyIndex();
...
class Header {
  constructor() {
    ...
    this.inHeader = (mouse) => {
      ...
      if (
        mouse.clientX < range.x ||
        mouse.clientY < range.y ||
        mouse.clientX > range.right ||
        mouse.clientY > range.bottom
      ) {
        this.close();
      }
    };
...
class Scroll {
  constructor() {
    ...
    this.checkTouchMove = (event) => {
      if (
        Math.abs(event.changedTouches[0].screenX - this.touchX) > 50 &&
        !this.reallyUp
      ) {
        this.notMoveY = true;
      }
      ...
      if (
        this.intop ||
        getElement("article").getBoundingClientRect().top >= 0
      ) {
        this.reallyUp = true;
...
class ColorMode {
  constructor() {
    ...
    this.change = () => {
      ...
      background.innerHTML = `<div style='background: var(--${
  ...
  constructor() {
    ...
    this.setHTML = () => {
      ...
      this.search.forEach((item) => {
        try {
          this.elements.push(
            new Pair(getElement(`#${item}`), getElement(`.${item}-sel`))
          );
        } catch (e) {}
      });
